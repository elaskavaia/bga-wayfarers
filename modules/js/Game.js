...
`;
    ...
    setupJournalConnectors() {
        for (const key of Object.keys(this.gamedatas.token_types)) {
            if (!key.startsWith("jconn_") || !key.endsWith("_0"))
                continue;
    ...
    setupGuildHotzoneTooltips() {
        ...
        const actions = {
            yellow: _("Trade — Once per turn, spend 1 Yellow Influence to Modify Dice in your supply by ±2 (one Die by ±2 or two Dice by ±1 each)."),
            blue: _("Exploration — Once per turn, spend 1 Blue Influence to hire a Ship — acts as if a placed Die had a Ship asset. Temporary effect."),
        ...
        for (const color of ["yellow", "blue", "black"]) {
            ...
            this.addTooltipHtml(`guild_${color}_action`, this.getTooltipHtml(_("Guild Action"), this.ttSection(_("Effect"), actions[color]) +
                this.ttSection(_("How to use"), _("Available as a button in the toolbar when it's your turn."))), this.defaultTooltipDelay);
        }
    }
     ...
     */
    updateCaravanCellTooltips(caravanId) {
        $(caravanId)
            ?.querySelectorAll(".ccell")
            .forEach((node) => {
            const r = node.dataset.r;
            const registered = gameui.tooltips?.[node.id] !== undefined;
            ...
            if (registered)
                return; // the text is static, re-registering would stack tooltips on the same node
    ...
    getPlaceRedirect(tokenInfo, args = {}) {
        ...
        if (tokenId.startsWith("card")) {
            ...
            if (leftCaravan)
                result.onEnd = () => this.updateCaravanCellTooltips(leftCaravan);
            if (location.startsWith("tableau")) {
                ...
                if (pos <= 0) {
                    ...
                    result.onEnd = (node) => {
                        ...
                        if (leftCaravan && leftCaravan !== `caravan_${color}`)
                            this.updateCaravanCellTooltips(leftCaravan);
                    };
                }
            }
    ...
    updateTokenDisplayInfo(tokenInfo) {
        ...
        switch (mainType) {
            ...
            case "jconn": {
                ...
                tokenInfo.tooltip = this.ttSection(_("Requirement"), this.getJournalRequirementTr(requirement, this.getRulesFor(connector, "gw", 1)));
                tokenInfo.tooltip += this.ttSection(_("Restriction"), _("Your Player Marker can only cross this splotch if you meet the requirement"));
                return;
            }
            case "action": {
    ...
    getJournalRequirementTr(requirement, count) {
    ...
    splitRequirementList(requirement, prefixLength) {
        return requirement
    ...
    showScoreDialog() {
        ...
        if (this.scoreDialog && $("popin_score_dialog"))
    ...
    setupScoreDialogTrigger(playerId) {
        const node = $(`player_score_${playerId}`);
        if (!node || !this.gamedatas.endScores)
            return;
