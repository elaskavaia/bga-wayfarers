...
function genbody($incsv, $conout) {
    ...
    while (($line = fgets($ins)) !== false) {
        ...
        $con = get_field("CON", $fields);
        if ($con) {
            $con = varsub($con, $fields);
            $con = preg_replace("/\W/", "_", $con);
            $con = strtoupper($con);
            $value = $map["num"];
            fwrite($conout, "  define(\"$con\", $value);\n");
            $map["CON"] = null;
        }
        ...
        foreach ($map as $key => $value) {
            ...
            if ($key == "php") {
                fwrite($out, "{$tabstop}{$tabstop}$value,\n");
                continue;
            }
...
function insertFileInMarkup($module, $matfile, $insertfile) {
    ($in = fopen($matfile, "r")) or die("Unable to open file! $in");
    ...
    if (!$found) {
        echo "Error: missing markup $markup_begin in $matfile\n";
        unlink($outfile);
        exit(2);
    }
    // generate data
    if ($insertfile) {
        ($ins = fopen($insertfile, "r")) or die("Unable to open $insertfile file!");
        while (($line = fgets($ins)) !== false) {
            fwrite($out, $line);
        }
        fclose($ins);
...
foreach ($args as $incsv) {
    ...
    $matfile = dirname("$incsv") . "/../modules/php/Material.php";

    echo "Reading $incsv\n";
    $outfile = $matfile . ".out.part";
    $conoutfile = $matfile . ".out.cpart";
    global $out;
    ($out = fopen($outfile, "w")) or die("Unable to open file! $outfile");
    ($conout = fopen($conoutfile, "w")) or die("Unable to open file! $conoutfile");
    if (!$skipoption) {
        genbody($incsv, $conout);
    }
    fclose($out);
    fclose($conout);
    insertFileInMarkup($module, $matfile, $outfile);
    clearstatcache();
    if (filesize($conoutfile)) {
        insertFileInMarkup("CON_" . $module, $matfile, $conoutfile);
    }
    unlink($outfile);
    unlink($conoutfile);
}
echo "Generated => $matfile\n";
