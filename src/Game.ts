...
`;
  setup(gamedatas) {
    try {
      ...
      document.querySelectorAll(".deck").forEach((node: Element) => this.updateTooltip(node.id, undefined, { force: true }));
  ...
  getUpgradeTileCopies(tokenId: string): string[] {
  ...
  async placeTokenServer(tokenId: string, location: string, state?: number, args?: any) {
    ...
    if (getPart(tokenId, 0) != "upg") return;
    for (const key of this.getUpgradeTileCopies(tokenId)) {
      if (key != tokenId) this.updateTooltip(key, undefined, { force: true });
    }
  }
    ...
    switch (mainType) {
      ...
      case "deck": {
        ...
        if (ctype === "land" || ctype === "water") {
          tokenInfo.tooltip += this.ttSection(
            _("Draw 3 Option"),
            _("When acquiring a Card via Dice placement, may forgo the faceup Card to draw 3 and keep 1 — costs 1 Provision or 1 Pigeon.")
          );
        }
        tokenInfo.tooltip += this.ttSection(
          _("Card Refreshing"),
          _(
            "Faceup Cards are refreshed only at the end of each player's turn. Remaining Cards slide to fill gaps before new Cards are revealed."
          )
        );
        return;
      }
  ...
  bgaFormatText(log: string | NotificationMessage, args: any): NotificationMessage {
    ...
    return { log: log as string, args: {} }; // no args - to prevent framework doing nasty stuff
  }
}
